...
for fn in ['admin.html','subadmin.html']:
 ...
 # replace first event object declaration after addEventItem
 pos=s.find('      window.addEventItem = function() {')
 obj=s.find('var ev = {',pos)
 if obj!=-1:
  namepos=s.find('name:',obj)
  if 'id: editingEventId' not in s[obj:namepos]:
   s=s[:namepos]+'id: editingEventId || undefined,\n          '+s[namepos:]
 # Add edit button
 if fn=='admin.html':
  marker="'<button class=\"admin-ticket-copy\" onclick=\"notifySubscribers(\\'" 
  pos=s.find(marker)
  if pos!=-1:
   insert = ''''<button type=\"button\" class=\"admin-ticket-copy\" onclick=\"editManagedEvent(\\'" + esc(id) + "\\')\" title=\"Edit event\">✏️</button>' +\n                '''
